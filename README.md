...
| 👁️ **进度监控** | 黑盒等待，终端只有干瘪的进度条 | **自动启动后台 HTTP 服务，提供实时可视化工作流画布** |
| 🧬 **稳定程度** | 多镜头容易变脸、变物、跳场景 | 角色/道具/场景参考图 + 链式承接，尽量稳住一致性 |
| ⏱️ **等待时间** | 多段视频通常一段段排队等 | 适合分镜时自动并行生成、并行审核 |
| 📚 **使用经验** | 每次踩坑都靠人记 | 成功经验和失败案例沉淀到 `skills/` 与分层记忆 |

---

## ✨ 九个值得 Star 的细节

### 1. 👁️ 告别黑盒等待：内置实时工作流画布
只要跑起任务，后台会自动启动一个常驻 HTTP 服务，并在终端输出链接。打开浏览器，你就能看到一个**自动刷新的可视化生产图谱**。从剧本拆解、故事板生成、视频渲染到 AI 审片，所有中间产物和进度状态一目了然。

### 2. 🎬 从「出一段」升级到「出一部」
现在的 AI 视频模型很强，但大多停在「一句话 → 一段素材」。真正要交付短片，还得拆镜头、保一致性、衔接镜头、审片、返工、拼接、配乐。VibeFilming 把这条链路全包了。

### 3. 🧙 告别 Prompt 玄学，内置专业“导演思维”
你不用背「大师级咒语」，也不用手调一堆模型参数。你说大白话，系统自动补齐主体定义、镜头语言、动作顺序。我们内置了**“四层质感框架”**（环境/镜头/材质/显影）和**“情绪微表情链路”**，确保生成的画面不仅好看，而且符合影视叙事逻辑。

### 4. ✏️ 不止会生成，改片也只用大白话
做出来不满意？不用回去重写 Prompt、不用学时间轴和剪辑软件。你直接说「第二镜慢一点」「把结尾的领养字幕去掉」「整体调暖一点」，Agent 自己判断该重生成哪一镜、该编辑还是重画、要不要重新拼接——**生成和修改，全程都是同一种「说人话」的体验**。

### 5. 🧠 它会做导演该做的决策
先做角色还是先做场景？该串行还是并行？哪一段需要重拍？哪些素材能复用？这些都交给 Agent 自己判断。它会强制构建**角色三视图**和**场景多视角包**作为全片真相源，锁死一致性。

### 6. 🧐 自带导演审片，不再靠抽卡出片
每张参考图、每段视频、每版成片都会进入 AI 审片。它不是简单打分，而是像导演一样把关：主角对不对、有没有越轴、情绪因果是否合理、四层质感是否达标。出片不再靠反复抽卡碰运气，而是有一个 AI 导演持续盯片、挑错、返工。
...
### 9. 🧱 架构开放，改装潜能巨大
它不是一个封死的黑盒。**决策内核与影视经验彻底解耦**：流程写在一篇篇 `SKILL.md` 里、能力做成可插拔的工具，想换模型只改一个字段。你完全可以把它改造成属于你自己风格的导演——加一条新拍法、定义你偏好的审片口味，**大多只需写 md、不碰内核代码，下次对话即生效**。
...
进控制台后直接说人话，同时可以**点击终端输出的链接打开实时画布**：

```
● 实时工作流看板 http://127.0.0.1:8765/canvas.html  (浏览器打开，随进度自动刷新)
...
### 1. 🧐 导演式自审与四层质感框架
每产出一个素材，AI 都会进行导演级审查。不是简单打分，而是按**环境、镜头、材质、显影**四层物理光学参数去检查画面，按**盲看观众视角**去查叙事和轴线匹配。
```
✅ 过审 → 进下一步
...
- **四层进化存储**：
  - **L1 索引层**：快速定位项目资产。
  - **L2 事实库**：沉淀锁定的角色特征与场景设定，防遗忘。
  - **L3 SOP 层**：将失败的教训转化为具体的“避坑指南”并写回文档（只有真正跑通的经验才准进记忆）。
  - **L4 存档层**：高度压缩的原始会话，保留决策轨迹。
...
| 📈 **越用越聪明** | 四层进化存储，踩坑写回文档，下次自动加载 |
| 🧐 **导演视角自审** | AI 遵循情绪链路和四层质感做发散性评判，拒绝无脑过审 |
...
编辑 `vibefilming.config.json` 里的 `ark.models`。文本、VLM、生图、生视频都能分别切换到你已开通的模型（火山方舟同时支持 DeepSeek 和豆包系列，一把 key 通用）：
