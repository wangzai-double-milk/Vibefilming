...
加工具现在只需要**一处**改动：在 `film/tools.py` 里写一个函数，挂上 `@film_tool` 装饰器。
装饰器会自动帮你做两件事：
...
> 特殊情况：某个参数想**不带 type**（允许多种类型，如 `vlm_understand` 的 `clip` 既可传
> 字符串也可传数组），就在 dict 里**不写 `type`** 即可：
> `"clip": {"description": "视频路径(str) 或图片路径列表(array)"}`
