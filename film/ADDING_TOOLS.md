...
假设要加一个音频响度分析工具 `my_audio_loudness`，贴进 `film/tools.py` 的「音频」区块：

```python
@film_tool(
    name="my_audio_loudness",
    desc="分析音频响度，返回 peak/lufs 等摘要",
    params={
        "audio": str,                                   # 必填、音频路径
        "name": str,
    },
...
def _my_audio_loudness(handler, args):
    return sdk.my_audio_loudness(args["audio"])
```

写完就行。**不用**再动 `tools_schema_film.json`。
...
print('总工具数:', len(t._DECORATED_TOOLS))
print('我的新工具已绑定:', hasattr(h, 'do_my_audio_loudness'))
print('schema 里有它:', any(s['function']['name']=='my_audio_loudness' for s in t.build_film_schema()))
"
```
