---
name: skill-audio
description: 音频与配乐技能。所有 shot 过审、concat 出成片之后要配 BGM，或要做台词/音效/配乐衔接/MV，或决定配不配 BGM 时读我。核心铁律：Seedance 只出对话+音效，BGM 一律走「成片完成 → BigMusic 出 BGM → 后期 audio_amix 整片铺底」，且 amix 后必须 vlm 审片。
---
...
4. 自己算 BGM 时长：用 `code_run` ffprobe 量 final_no_bgm.mp4 的秒数，clamp 到 [30,120]
   （火山 GenBGM 硬限制 30-120s，超过 120s 也只能给 120s）。
   gen_audio_bgm(prompt=<≥50字风格描述>, duration=<上面算出的秒数>, name="bgm_main")
...
review_final_with_bgm 的 question 必须覆盖以下 4 项（参考 [skill_director_vlm](../skill_director_vlm/SKILL.md) 三段式模板）：
...
│  ④ 需要 + 自动生成（等长直出，不做循环）：              │
│     先用 code_run ffprobe 量 final_no_bgm 秒数,clamp[30,120]│
│     gen_audio_bgm(                                      │
...
q1 = gen_video_t2v(
    ...
    name="shot_s01",
    reference_images=[bunny_ref_url],
    generate_audio=True,
    duration=8,
)
...
│   │   ├── code_run ffprobe 量 final_no_bgm 秒数, clamp[30,120]
│   │   ├── gen_audio_bgm(prompt, duration=<量出秒数>, name)
