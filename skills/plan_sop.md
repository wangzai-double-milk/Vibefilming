...
> **兼容性说明**：本 SOP 是仓库内部旧版 plan-mode 工作法。
> 具体工具名和 subagent 启动方式可能随运行环境变化。
> 执行时以当前系统/开发者指令和当前可用工具为最高优先级。
> 涉及 `code_run`、`file_read`、`file_patch` 或强制 subagent 的写法，只能在当前环境确实支持且被授权时使用。
> 当前工具不支持时，用等价的可用读写/验证工具完成同一目标。
...
3. **检查标记**：
   - `[D]` 标记：可用且被授权时委托 subagent 执行，主 agent 只收结果摘要；不可用时拆小由主 agent 执行并记录原因。
   - `[P]` 标记：读 subagent.md 执行 Map 模式。
   - `[?]` 条件：评估条件选分支，未选标 `[SKIP]`。
