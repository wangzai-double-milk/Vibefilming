...
> **兼容性说明**：本 SOP 是仓库内部旧版 plan-mode 工作法，具体工具名和 subagent 启动方式可能随运行环境变化。执行时以当前系统/开发者指令和当前可用工具为最高优先级；涉及 `code_run`、`file_read`、`file_patch` 或强制 subagent 的写法，只能在当前环境确实支持且被授权时使用。当前工具不支持时，用等价的可用读写/验证工具完成同一目标。

**触发**：3步以上有依赖/多文件协同/条件分支/需并行 | **禁用**：1-2步简单任务直接做
任务开始前建议创建工作目录 `./plan_XXX/`（XXX=任务英文短名），用于保存计划、探索记录和验证证据。
...
- 先探测再规划，禁止凭空写计划。
- subagent 只在当前环境支持、且用户/系统授权时使用；不满足条件时，主 agent 可直接用只读工具探测。
- 探索阶段默认只读，禁止修改文件或执行有副作用的操作。
- subagent 启动失败时，排查原因后最多重试 2 次；仍不可用就回退为主 agent 只读探测，并在计划里标明验证责任由主 agent 承担。

**目标**：在写任何计划之前，搞清3件事：
① 环境现状（有什么、缺什么） ② 可用SOP ③ 关键不确定点

**为什么优先考虑 subagent**：主 agent 上下文是稀缺资源，探测长输出会挤占规划执行空间。但这只是成本优化，不得覆盖当前环境的工具授权规则。
...
### 步骤2：按需启动探索 subagent（监察模式）

当前环境支持并允许 subagent 时，按 subagent.md 启动探索 subagent，**加 `--verbose`** 开启监察模式；否则由主 agent 用当前可用工具直接完成同样的只读探测。input 要点：
...
使用 subagent 时，主 agent 主动观察 output.txt 进度（`--verbose` 输出含原始工具结果），而非无脑 sleep 轮询：
...
N+1. [ ] **[VERIFY] 启动独立验证subagent**
     ...
     操作：读plan_sop.md第四章内容 → 准备verify_context.json → 可用且被授权时启动验证 subagent，否则用当前可用工具做独立验证 → 读取/产出 VERDICT → 按结果处理
     ⚠ 不可跳过验证；没有 subagent 时也要留下实际工具证据，不能只写文字判断。
...
- □ **复杂/繁琐步骤是否标注了[D]？**（读大量代码/网页/重复操作在可用且被授权时优先委托 subagent；否则拆小由主 agent 执行）
...
3. **检查标记** — `[D]` 标记 → 可用且被授权时委托 subagent 执行，主 agent 只收结果摘要；不可用时拆小由主 agent 执行并记录原因；`[P]` 标记 → 读 subagent.md 执行 Map 模式；`[?]` 条件 → 评估条件选分支，未选标 [SKIP]
...
> 全部步骤 [✓] 后进入。优先启动独立 subagent 做对抗性验证，避免上下文污染；当前环境不支持或未授权 subagent 时，用当前可用工具做对抗性验证，并把证据写清。

### 触发条件

- 所有执行步骤标记为 `[✓]`
- 所有 plan 模式任务都必须有验证证据；能用 subagent 时优先独立验证，不能用时由主 agent 运行实际检查。
...
可用且被授权时按 subagent.md 标准流程启动验证 subagent；否则主 agent 按 verify_sop.md 用当前可用工具执行同等检查。input 要点：

- **角色**：你是独立验证者，工作是对抗性验证（证明交付物不能用）
- **第一步强制**：完整阅读 verify_sop.md（用当前可用读文件工具）
...
FAIL → 提取具体失败项 → 回执行态修复（不重新规划） → 修复完成 → 再次执行验证（可用且被授权时用 subagent，否则主 agent 运行实际检查） → 最多 2 轮 FAIL-重试，超过 ask_user 介入
...
浏览器/键鼠/定时任务等场景：主 agent 执行操作并导出证据（截图/录屏/日志）→ 可用且被授权时让 subagent 验证证据文件；否则主 agent 必须基于证据文件和工具输出给出 PASS/FAIL，不能凭感觉判断。
