---
name: skill-video-chain
description: 调 gen_video_t2v 之前，只要项目里 ≥2 段视频必读；镜头衔接/拼接帧裁剪/防画质劣化时。 管多段视频如何链式承接、拼接前的帧裁剪铁律、防止画质逐段劣化。
---
...
> **配套**：音频连贯性详见 [skill_audio.md](../skill_audio/SKILL.md)（reference_audios 共享 / MV 模式 / 4 类特殊字符）
...
- 入参 = `prompt` + `reference_images` + `reference_video_url`
...
gen_video_t2v(
    ...
    reference_images=[本镜出现的角色/道具/场景参考图 url],   # 用 gen_image 出好的那几张
    duration=6,
    name="shot_s01",
)
```
...
gen_video_t2v(
    ...
    reference_images=[本镜出现的角色/道具/场景参考图 url],
...
reference_images 上限 9 张。一个角色的 turnaround 默认三视图 = 1 张（含 front/side/back 合一）。第 N 段（N≥2）的标准配置：

| 项目 | 数量 |
|---|---|
| 本镜角色/道具/场景的所有参考图（reference_images） | 1-4 张（character × 1 turnaround + props/scene 各 1） |
...
详细调度策略见 [skill_async_schedule.md](../skill_async_schedule/SKILL.md)。

---

## 配乐策略（搭配链式衔接的关键）

> **完整音频规范见 [skill_audio.md](../skill_audio/SKILL.md)。** 这里只列项目级铁律。
...
- ✅ 正确做法：
  ...
  - 失败时**先诊断不要降级**：读错误 → 排查参数（详见 [skill_self_decision.md](../skill_self_decision/SKILL.md) "失败不许降级"）
  - 严禁用 `video_speed` 慢动作凑时长——分镜规定几段就拍几段

### Case B · 30s 小猫跳舞 part1+part2（早期版本）

- ❌ 实际做法：part1 和 part2 都 `reference_video_url=null`，各喂一份角色 turnaround 参考图
- 后果：part2 开头橘猫姿态/位置/光影完全不接 part1 结尾；两段原生音节拍不统一硬拼违和
- ✅ 正确做法：part2 必须传上段 url；**Seedance 只出对白+音效不出 BGM**，整片拼好后再统一 `audio_amix` 铺一首 BGM（详见 [skill_audio.md](../skill_audio/SKILL.md)）
...
3. 同时把**高清的人物参考图 + 高清场景参考图**继续放进 `reference_images`
4. 模型用白模学**结构 / 运动 / 镜头**，用高清参考图学**外观 / 颜色 / 纹理** → 输出画质重新对齐基准，不再劣化
